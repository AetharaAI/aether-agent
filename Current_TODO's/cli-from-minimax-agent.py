...
# UAP Integration (optional)
try:
    from mini_agent.uap_integration import UAP
    UAP_AVAILABLE = True
except ImportError:
    UAP_AVAILABLE = False
    UAP = None
...
async def run_agent(workspace_dir: Path):
    ...
    # 8. Initialize UAP integration (optional - connects to Triad grid)
    if UAP_AVAILABLE:
        try:
            UAP.initialize(
                agent_name="flux-minimax",
                capabilities=["reasoning", "coding", "file-operations", "bash-execution"],
            )
        except Exception as e:
            print(f"{Colors.YELLOW}⚠️  UAP initialization skipped: {e}{Colors.RESET}")
    ...
    while True:
        try:
            ...
            # Publish response to UAP (if enabled)
            if UAP_AVAILABLE and UAP.is_enabled():
                UAP.publish_response(response or "")
    ...
    # 12. Cleanup MCP connections and UAP
    try:
